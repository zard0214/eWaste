...
import {
    ...
    getBrands,
    getProductV2ById,
    getSeries, getStoreById, saveBrands, saveStores, updateBrands,
    updateProductV2, updateStores,
    uploadFile
} from "../util/APIUtils";
...
const BrandsFormV2 = (props) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (product.id) {
            ...
            saveStores(product)
                .then(() => {
                    Alert.success("Store insert successfully");
                    props.history.push("/stores")
                })
                .catch(error => {
                    submitBtn.disabled = false;
                    cancelBtn.disabled = false;
                    Alert.error("Failed to insert store");
                });
        }
    ...
    return (
        <div>
            ...
            <div className="content-container">
                <div className="card">
                    {/*<Skeleton>*/}
                    <h2 className="form-title">{state === undefined ? "New Store Details" : `Store ID: ${product.id} Details`}</h2>
                    <Divider></Divider>
                    <div className="form-container">
                        <Form labelCol={{span: 4}} wrapperCol={{span: 17}} layout="horizontal" filled>
                            ...
                            <Form.Item required={true} label="Store Image">
                                {/*{product.imageUrl ?*/}
                                <div>
                                    <Image
